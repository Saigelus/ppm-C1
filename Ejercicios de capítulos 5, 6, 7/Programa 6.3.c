...
tridimensional se necesitan tres ciclos para recorrer el arreglo. */
{
    ...
    for (K=0; K<PR; K++)
        for(I=0; I<FI; I++)
            for(J=0; J<CO; J++)
            {
                printf("Año: %d\tSemestre: %d ", K+1, I+1, J+1);
                scanf("%d", &A[I][J][K]);
            }
}
...
/*Esta función se utiliza para determianr el año en el que la carrera
Ingenier+ia en computación recibió el mayor núero de alumno. Observ que no 
